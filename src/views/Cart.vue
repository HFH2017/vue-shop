<template>
    <div>
                            ...
                            <li v-for="item in cartList">
                                <div class="cart-tab-1">
                                    <div class="cart-item-check">
                                        <a href="javascipt:;" class="checkbox-btn item-check-btn" v-bind:class="{check:item.checked=='1'}" @click="edit('',item)">
                                    ...
                                    <div class="item-quantity">
                                        <div class="select-self select-self-open">
                                            <div class="select-self-area">
                                                <a class="input-sub" @click="edit('sub',item)">-</a>
                                ...
                                <div class="cart-tab-5">
                                    <div class="cart-item-opration">
                                        <a href="javascript:;" class="item-edit-btn" @click="popModal(item.productId,item.productNum)">
                ...
                <div class="cart-foot-wrap">
                    <div class="cart-foot-inner">
                        ...
                        <div class="cart-foot-r">
                            ...
                            <div class="btn-wrap">
                                <a class="btn btn--red" @click="toAddress" :class="{'btn--dis':this.checkedCount===0}">Checkout</a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <modal v-bind:showModal="showDelModal" @close="cancelDel">
            <p slot="message">确认删除吗？</p>
            <div slot="btn">
                <a href="javascript:; " class="btn btn--m" @click="delCart()">确认</a>
                <a href="javascript:; " class="btn btn--m">取消</a>
            </div>
        </modal>
...
export default {
    ...
    methods: {
        init() {
            axios.get('/users/getcartlist').then((res) => {
        ...
        popModal(id,num) {
            ...
            this.curProductNum = num
        },
        cancelDel() {
            this.showDelModal = false
        },
        delCart(item) {
            axios.post('/users/delcart', { productId: this.curProductId }).then((res) => {
                if (res.data.status === '0') {
                    this.init()
                    this.$store.commit("updateCartCount",-this.curProductNum)
                } else {
                    alert('出错了')
                }
            })
        ...
        edit(flag, item) {
            if (flag === 'add') {
                ...
                if (item.productNum <= 1) { return } else {
                    item.productNum--
                    this.$store.commit("updateCartCount",-1)
                }
            } else {
        ...
        changeCheckAll(item) {
            ...
            this.cartList.forEach(function (item) {
                flag ? item.checked = '1' : item.checked = '0'
            });
            axios.post('/users/changecheckall', { checkall: this.checkAll }).then((res) => {
                console.log(res)
            });
        },
        toAddress() {
