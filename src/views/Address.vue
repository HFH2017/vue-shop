        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for = "(item,index) in filterAddressList" @click="choose(index,item.addressId)" :class="{'check':index===checkIndex}">
                    <dl>
                      <dt>{{item.userName}}</dt>
                      <dd class="address">{{item.streetName}}</dd>
                      <dd class="tel">{{item.tel}}</dd>
                    </dl>
                    <div class="addr-opration addr-del" @click="popModal(item.addressId)">
              ...
              <div class="shipping-addr-more">
                <a class="addr-more-btn up-down-btn" href="javascript:;" @click="showMore" :class="{'open':limit===3}">
                  more
                  <i class="i-up-down">
            ...
            <div class="next-btn-wrap">
              <router-link v-bind:to="{path:'orderconfirm',query:{'addressId':selectAddrId}}" class="btn btn--m btn--red">Next</router-link>
            </div>
          </div>
        </div>
      </div>
      <modal v-bind:showModal="showModal" @close="closeModal">
          <div slot="message">确定要删除地址吗？</div>
          <div slot="btn">
            <a class="btn btn--m" href="javascript:;">取消</a>
            <a class="btn btn--m" @click="delAddress" href="javascript:;">确定</a>
          </div>
      </modal>
...
<script>
    ...
    export default{
        ...
        methods:{
            ...
            showMore(){
               this.limit==3 ? this.limit = this.addressList.length : this.limit=3
            },
            choose(index,id){
                this.checkIndex=index;
                this.selectAddrId = id
            },
